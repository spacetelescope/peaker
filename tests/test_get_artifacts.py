...
from peaker.get_artifacts import get_artifacts, _filter_artifacts
...
def test_fake_credentials(tmpdir):
    ...
    pytest.raises((requests.exceptions.ConnectTimeout, pyartifactory.exception.ArtifactoryError),
                  lambda: get_artifacts(fake_credentials_file, REPO, PY_VERSION,
                                        start_date=START_DATE, end_date=END_DATE))

@dataclass
class Art:
    # Mock the Artifactory object
    uri: str
    created: datetime
    files: list
...
class ArtifactListFolderResponse:
    # Mock the folder attribute in the Artifactory object
    uri: str
    size: int
...
class ArtifactListFileResponse:
    # Mock the file attribute in the Artifactory object
    uri: str
    size: int
    lastModified: datetime
    folder: bool
